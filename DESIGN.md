...
Home は個人の入口であり、ランディングページではない。短い自己紹介の下に、Notes の新着を日付順で静かに並べる。名前は必要以上に大きくせず、読み始めるための小さな標識として扱う。

基本構成:

1. Header は site 名として `yona.dev` だけを置く。
2. Home の主見出しで `Koh Yonamine` を一度だけ示す。
...
ハリネズミは元の icon 写真を小さな丸い mark として使ってよい。名前の横に置く場合も、本人由来の小さなサインに留め、プロフィール写真のように大きく扱わない。
