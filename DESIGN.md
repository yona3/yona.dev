...
ベースの本文・見出し・ナビゲーション・日付・種別ラベルは OS 標準の sans-serif (`system-ui` を先頭にしたフォールバック) に統一する。serif は使わず、読みやすさと普段着の温度を優先する。読みにくい装飾書体は採用しない。

吹き出し (`.nameSpeech`) の中だけ M PLUS Rounded 1c を使い、丸い文字で「触れた人だけが気づく小さな余韻」を演出する。同じ丸文字をサイト全体には広げず、対比で吹き出しを持ち上げる。
...
小さなサインは隠し味として使う。hover や focus でわずかに反応してよいが、動きは原則 `transform` と `opacity` に限定し、本文を読む流れを邪魔しない。

例外として、Home の吹き出しでは初回表示の type 描画 (DOM テキストの逐次更新) を採用し、ハリネズミが話しかけてくる小さな演出として扱う。これは隠し味として 1 箇所に限定し、他の本文 / 一覧 / metadata では用いない。

ハリネズミの click インタラクション (画面外を一周して戻る lap) と hover の控えめな wobble も、user-visible な遊びとして許容する。動きは `transform` と `opacity` で表現し、`prefers-reduced-motion: reduce` 環境ではすべて停止 / no-op とする。
