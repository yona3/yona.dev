...
| 軸 | 目的 | 書き込む section |
| --- | --- | --- |
| 目的 | ユーザーから見える成果を明確にする | `目的と全体像` |
| 制約 | 変えてはいけないものを明確にする | `依存関係と契約`, `冪等性と復旧` |
| 受け入れ条件 | 成功の観測方法を決める | `検証と受け入れ条件` |
| scope 境界 | 対象 file / module / route と対象外を分ける | `具体手順`, `依存関係と契約` |
| tradeoff | 競合時に何を優先するか決める | `判断記録` |
...
4. Autonomous phase: 承認 scope 内で実装する。
5. Approval gate 2: evidence と残リスクを報告する。
...
- `具体手順` では file、function、module、type を一意に指せる名前で書きます。
...
| Section | 目的 | 機械検査 | 書き方 |
| --- | --- | --- | --- |
| `目的と全体像` | 変更前後のユーザーから見える挙動を説明する | 見出し必須 | 内部属性だけで終えない |
| `進捗` | timestamp 付き checkbox で状態を残す | timestamp format | 日本時間で書く |
| `気づきと発見` | 発見と proof を残す | `Observation:` / `Evidence:` | transcript や file-scoped diff を貼る |
| `判断記録` | 判断理由を残す | `Decision:` / `Rationale:` / `Date/Author:` | tradeoff を書く |
| `依存関係と契約` | dependency と成立条件を明示する | `Dependency:` / `Reason:` / `Contract:` | file/module contract を書く |
| `具体手順` | 実装手順を具体化する | `Working directory:` / `Command:` / `Expected outcome:` | numbered list を使う |
| `検証と受け入れ条件` | 成功と失敗を観測可能にする | `Input:` / `Observe:` / `Failure signal:` | 完了時は actual proof に置き換える |
| `冪等性と復旧` | retry と cleanup を示す | 見出し必須 | 5 要件を満たす |
| `未完了事項` | 残作業と残リスクを示す | 見出し必須 | なければ `None.` |
